...
def construct_paired_msa(  # noqa: C901, PLR0915, PLR0912
    ...
    # make sure the asym_id is a list of non-decreasing integers
    ...
    for chain_id, chain_msa in msa.items():
        ...
        # sequences["taxonomy"] != -1 are the results of paired sequences
...
def process_token_features(
    ...
    res_type = from_numpy(token_data["res_type"]).long()        # Here we may need cal again ?
    res_type = one_hot(res_type, num_classes=const.num_tokens)
    disto_center = from_numpy(token_data["disto_coords"])       # Both zero ?
    ...
    # Token bond features   (We don't need now.)
    ...
    # Pocket conditioned feature  (We don't need now, may be later vesion)  
    pocket_feature = (
    ...
    if inference_binder is not None:
        ...
        if len(binder_asym_ids) > 0:
            ...
            binder_coords = np.concatenate(binder_coords, axis=0)   # All zero.
    ...
    # Here we may only consider the residues type if before calculator.
    token_features = {
