...
class ConsecutiveCA(StaticFilter):
    ...
    def filter(self, structure: Structure, H_chain_id: int, L_chain_id: int) -> np.ndarray:
        ...
        antibody_chains = set([H_chain_id, L_chain_id])

        # Remove chain if consecutive CA atoms are above threshold
        for i, chain in enumerate(structure.chains):
            # Only consider antibody chains
            if chain["asym_id"] not in antibody_chains:
                continue
...
class ClashingChainsFilter(StaticFilter):
    ...
    def filter(self, structure: Structure) -> np.ndarray:  # noqa: PLR0912, C901
        ...
        # TODO: Check this code, May have prbolems?.
        # The pairs is empty.
        pairs = zip(range(num_chains), range(num_chains))
        pairs = [(i, j) for i, j in pairs if i < j]
        
        # Right may is this.
        # Now we don't execute it.
        # pairs = list(combinations(range(num_chains), 2))
