...
ARIA_NUM_CONN=8
WORKDIR="${1:-$(pwd)}" # if argument is not provided, use current directory
...
if [ ! -f UNIREF30_READY ]; then
  if [ ! -f "${UNIREF30DB}.tar.gz" ]; then
    echo "File ${UNIREF30DB}.tar.gz not found. Downloading..."
    downloadFile "https://wwwuser.gwdg.de/~compbiol/colabfold/${UNIREF30DB}.tar.gz" "${UNIREF30DB}.tar.gz"
  else
    echo "File ${UNIREF30DB}.tar.gz already exists. Skipping download."
  fi
...
if [ ! -f COLABDB_READY ]; then
  if [ ! -f "colabfold_envdb_202108.tar.gz" ]; then
    echo "File colabfold_envdb_202108.tar.gz not found. Downloading..."
    downloadFile "https://wwwuser.gwdg.de/~compbiol/colabfold/colabfold_envdb_202108.tar.gz" "colabfold_envdb_202108.tar.gz"
  else
    echo "File colabfold_envdb_202108.tar.gz already exists. Skipping download."
  fi
